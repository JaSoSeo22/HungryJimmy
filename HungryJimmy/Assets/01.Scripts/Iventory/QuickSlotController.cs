...
//     [SerializeField] private Transform tf_ItemPos;      // 아이템이 위치할 위치
...
//     // 필요한 컴포넌트
//     [SerializeField]
//     private GameObject go_Selectedimage;        // 선택된 퀵슬롯의 이미지 (하얀색 네모)
//     [SerializeField]
...
//     void Start()
//     {
//         quickSlots = tf_parent.GetComponentsInChildren<Slot>();     // 자식객체에서 넣어줌
//         anim = GetComponent<Animator>();
//         selectedSlot = 0;       // 초기화 시켜줌 
//     }
...
//     {
//         if (isCoolTime)     // isCoolTime = ture일때
//         {
//             currentCoolTime -= Time.deltaTime;      // 1초에 1씩 감소
...
//         {
//             if (Input.GetKeyDown(KeyCode.Alpha1))       // 1번 버튼 눌리면 ChangeSlot호출
//             {
//                 ChangeSlot(0);
//             }
...
//     {
//         if (selectedSlot == _num)       // selectedSlot와 활성회된 슬롯의 번호가 같으면
//         {
//             Execute();
//             return;
//         }
...
//     private void ChangeSlot(int _num)       // 넘어온 슬롯 번호
//     {
//         SelectedSlot(_num);

//         Execute();      // 퀵슬롯에 담긴 내용 실행
//     }
...
//         if (quickSlots[selectedSlot].item != null)      // 퀵슬롯의 아이템이 null이 아닌지
//         {
//             if (quickSlots[selectedSlot].item.itemType == Item.ItemType.Equipment)      // 장비아이템
//             {
...
//             }
//             else if (quickSlots[selectedSlot].item.itemType == Item.ItemType.Used)      // 소모품인 경우
//             {
//                 ChangeHand(quickSlots[selectedSlot].item);      // 현재 선택된 퀵슬롯의 아이템 넘겨줌
//             }
//             else
//             {
//                 ChangeHand();
...
//         }
//     }
//     // private void ChangeHand(Item _item = null)      // 넘어오는 파라미터 아이템타입 줌
//     // {
//     //     StartCoroutine(theWeaponManager.ChangeWeaponCoroutine("HAND", "맨손"));
//     //     if (_item != null)      // 넘어온 아이템 파라미터가 null이 아닌경우
//     //     {
//     //         StartCoroutine(HandItemCoroutine());        // HandItemCoroutine()실행
//     //     }
//     // }
...
//     //     go_HandItem = Instantiate(quickSlots[selectedSlot].item.itemPrefab, tf_ItemPos.position, tf_ItemPos.rotation);      // 지정한 위치에 아이템 생성
...
//     //     go_HandItem.tag = "Untagged";       // 획득되지 않도록 태그 바꿔줌
//     //     go_HandItem.layer = 7;      //Weapon 레이어
//     //     go_HandItem.transform.SetParent(tf_ItemPos);        // 부모객체 따라다니게 함
//     // }

//     public void EatItem()
//     {
//         CoolTimeReset();
//         quickSlots[selectedSlot].SetSlotCount(-1);      // 선택된 슬롯에서 -1씩해줌

//         if (quickSlots[selectedSlot].itemCount <= 0)        // 0보다 작거나 같을때
//         {
//             Destroy(go_HandItem);       // 생성된 아이템 파괴
//         }
//     }
